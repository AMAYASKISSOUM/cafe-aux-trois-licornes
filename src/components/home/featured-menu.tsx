  ...
  return (
    <section className="bg-petrol-tint py-20 sm:py-28">
      <Container>
        ...
        <RevealGroup className="mt-12 grid grid-cols-2 gap-x-6 gap-y-10 sm:grid-cols-3 lg:grid-cols-6">
          {items.map((item) => {
            const name = localized(item.name, locale);
            return (
              <RevealItem key={item.slug} variant="card" className="flex flex-col gap-3">
                <Photo
                  src={item.imageUrl}
                  alt={name}
                  label={name}
